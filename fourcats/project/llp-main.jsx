...
function App() {
  ...
  useEffect(() => {
    ...
    r.setProperty(
      "--font-d",
      `${FONTS[t.headlineFont] || "'Quicksand'"},system-ui,sans-serif`,
    );
  }, [t]);
  ...
  const finishOnboarding = (d) => {
    if (d && d.name) {
      ...
      const np = {
        ...
        species: d.species,
        breed: d.breed || "Mixed",
        age: `${d.age} ${d.age === 1 ? "yr" : "yrs"}`,
        weight: `${(+d.weight).toFixed(1)} kg`,
        area: d.area,
        tags: (d.conditions && d.conditions.length
          ? [d.conditions[0]]
          : ["Lovely"]
        ).concat(homeTag),
      };
  ...
  useEffect(() => {
    if (!pets.find((p) => p.id === active.id)) setActive(pets[0]);
  }, [pets]);
  ...
  const META = {
    ...
    discovery: {
      ...
      sub: `Services near ${active.area}`,
    },
    profiles: {
      title: "Pet Profiles",
      sub: `${pets.length} lovely pet${pets.length > 1 ? "s" : ""}`,
    },
    designsystem: {
      title: "Design System",
      sub: "Modern Warmth · standardised",
    },
  };
    ...
    <React.Fragment>
      ...
      <TweaksPanel title="Tweaks">
        ...
        <TweakColor
          label="Accent"
          value={t.accent}
          options={ACCENTS.map((a) => a.primary)}
        ...
        <TweakSlider
          ...
          step={10}
          unit="%"
          onChange={(v) => setTweak("cuteness", v)}
        />
        <TweakSelect
          label="Headline font"
          value={t.headlineFont}
          options={["Quicksand", "Nunito", "Baloo 2"]}
