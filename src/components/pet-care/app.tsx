...
function MobileSidebar({ active }: { active: Section }) {
  return (
    <details className="fixed left-4 top-4 z-50 md:hidden">
      <summary className="flex size-11 cursor-pointer list-none items-center justify-center rounded-full bg-white text-[#9c3f53] shadow-lg [&::-webkit-details-marker]:hidden">
        <Menu className="size-5" />
      </summary>
      <div className="mt-3 w-[calc(100vw-2rem)] max-w-80 rounded-2xl border border-[#dac0c3]/60 bg-[#f3f4f5] p-4 shadow-xl">
        <div className="mb-4 flex items-center gap-3 border-b border-[#dac0c3]/40 pb-4">
          <Avatar className="size-12 border-2 border-[#ff8da1] p-1">
            <AvatarImage
              alt="Little Lovely Pets profile"
              className="rounded-full object-cover"
              src={petPortrait}
          ...
          <div>
            <h1 className="font-[family-name:var(--font-brand)] text-xl font-bold text-[#9c3f53]">
              Little Lovely Pets
            </h1>
            <p className="text-xs text-[#554244]">Singapore AI Care</p>
          </div>
        </div>
            ...
            return (
              <Link
                className={cn(
                  ...
                  selected ? "bg-[#ff8da1] text-[#782338]" : "text-[#554244]",
                )}
                href={item.href}
                key={item.id}
              >
                <Icon className={cn("size-5", selected && "fill-current")} />
                {item.label}
              </Link>
            );
          })}
...
export function Dashboard() {
  return (
    <main className="min-h-screen bg-[#f8f9fa] px-5 pb-12 pt-20 md:ml-64 md:px-16 md:pt-12">
...
export function Assistant() {
  return (
    <main className="grid min-h-screen bg-[#f8f9fa] pt-16 md:ml-64 md:pt-0 lg:grid-cols-[1fr_320px]">
