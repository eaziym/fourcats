import { tool } from "ai";
import { z } from "zod";
import { formatPriceRange } from "@/lib/pet-data/format";
import {
  postalToLatLng,
...
function placeToJson(p: ServicePlaceResult) {
  return {
    name: p.name,
    address: p.address,
    rating: p.rating,
...
export function buildPetTools(opts: {
  ...
  const { defaultPetType, petLatLng } = opts;
