...
import { Bot, ChevronDown, Info, Loader2, Map as MapIcon, SendHorizontal, ShoppingBag, UserRound } from "lucide-react";
        ...
        <PetCareShell active="assistant">
            <main className="grid min-h-screen bg-[#f8f9fa] md:ml-64 lg:grid-cols-[1fr_320px]">
                <section className="relative flex h-screen flex-col overflow-hidden bg-[radial-gradient(#dac0c3_1px,transparent_1px)] [background-size:34px_34px]">
                    ...
                    <div ref={scrollRef} className="flex flex-1 flex-col gap-6 overflow-y-auto px-5 py-8 pb-4 md:px-10">
                        <div className="mx-auto w-full max-w-4xl">
                            <div className="mb-6 text-center">
                                <span className="rounded-full bg-[#edeeef] px-5 py-2 text-sm text-[#554244]">Today</span>
                            </div>
                    ...
                    <ChatInput input={input} busy={busy} onInputChange={setInput} onSubmit={handleSubmit} />
                </section>
...
function ModelSelector({ modelId, onModelChange }: { modelId: string; onModelChange: (id: string) => void }) {
                    ...
                    {availableModels.map((m) => (
                        <button
                            ...
                            className={`w-full rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                                m.id === modelId ? "bg-[#ff8da1]/20 font-medium text-[#782338]" : "text-[#554244] hover:bg-[#f3f4f5]"
                            }`}
                        >
                            {m.label}
                        </button>
        ...
        <div className="border-t border-[#dac0c3]/30 bg-gradient-to-t from-[#f8f9fa] via-[#f8f9fa] p-5 md:px-10">
            ...
            >
                <input
                    ...
                    disabled={busy}
                />
                <Button type="submit" className="size-11 rounded-full bg-[#9c3f53] disabled:opacity-50" size="icon" disabled={busy || !input.trim()}>
        ...
        <aside className="hidden border-l border-[#dac0c3]/40 bg-white px-8 py-8 lg:block">
            ...
            <div className="mb-6 flex items-center gap-3 rounded-xl border border-[#ffd167] bg-gradient-to-r from-[#ffdf9b]/50 to-white p-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-[#554244]">Shih Tzu • 4yo • Sensitive Skin</p>
                </div>
            </div>
            <div className="grid gap-4">
                {sources.map(({ source, title, detail, icon: Icon }) => (
                    <div className="rounded-2xl border border-border bg-muted/30 p-4" key={title}>
