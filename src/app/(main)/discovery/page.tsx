import { Cross, Heart, MapPin, Scissors, Search, SlidersHorizontal, Sparkles, Star, Tag } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Button } from "@/components/ui/button";
import { CardContent } from "@/components/ui/card";
import { Input } from "@/components/ui/input";
import { cn } from "@/lib/utils";
import { discoveryFilters, groomerInterior, mochiPortrait } from "@/lib/pet-data";
    ...
    return (
        <section className="z-10 flex w-full flex-col border-r border-[#dac0c3]/30 bg-white shadow-[4px_0_24px_rgba(29,53,87,0.08)] md:h-screen md:w-[500px]">
            <div className="border-b border-[#dac0c3]/30 p-5 md:p-8">
                <h2 className="font-[family-name:var(--font-brand)] text-4xl font-bold md:text-5xl">Local Discovery</h2>
                <p className="mt-1 text-lg text-[#554244]">Find trusted services in Singapore.</p>
                <div className="relative mt-5">
                    ...
                    />
                    <button className="absolute right-3 top-1/2 -translate-y-1/2 rounded-lg bg-[#edeeef] p-2 text-[#554244]" type="button">
    ...
    return (
        <div className="mt-4 flex items-center gap-3 rounded-xl border border-[#ffd167] bg-gradient-to-r from-[#ffdf9b]/50 to-white p-2">
            ...
            <div>
                ...
                <p className="text-xs text-[#554244]">Filtered for sensitive skin expertise</p>
            </div>
            <button className="ml-auto px-2 text-sm font-bold text-[#9c3f53]" type="button">
                Edit
            </button>
        </div>
    );
            ...
            {discoveryFilters.map(({ label, icon: Icon, active }) => (
                <button
                    className={cn(
                        ...
                        active ? "border-[#9c3f53] bg-[#9c3f53] text-white" : "border-[#dac0c3] bg-[#edeeef] text-[#191c1d]",
                    )}
                    key={label}
                    type="button"
                >
...
function ListingCard({ name, location, tag, rating, selected }: (typeof listings)[number]) {
    return (
        <SpotlightCard className={cn(selected && "bg-gradient-to-r from-white to-[#ffdf9b]/20")}>
            <CardContent className="flex gap-5 p-4">
                <div className="relative">
                    <img alt={name} className="size-28 rounded-lg object-cover" src={groomerInterior} />
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex justify-between gap-4">
                        <h3 className="font-[family-name:var(--font-brand)] text-2xl font-bold">{name}</h3>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                        <Pill className="rounded-md border border-[#ffd167] bg-[#ffdf9b]/70 text-[#d48700]">{tag}</Pill>
                        <Pill className="rounded-md bg-[#edeeef] text-[#554244]">HDB-Approved</Pill>
                    </div>
                    <div className="mt-4 flex items-center justify-between">
                        <span className="font-semibold text-[#9c3f53]">$$$</span>
                        <Button className="rounded-full bg-[#ffd9dd] text-[#9c3f53] hover:bg-[#ffb2bd]">Book Now</Button>
                    </div>
                </div>
            </CardContent>
        </SpotlightCard>
...
function SpecialOfferBanner() {
    return (
        <div className="relative overflow-hidden rounded-2xl bg-[#b0c7f1] p-6 text-[#18365f]">
            <Pill className="mb-4 rounded-md bg-[#1d3557] text-white">SPECIAL OFFER</Pill>
    ...
    return (
        <section className="relative h-[520px] flex-1 overflow-hidden bg-[#8fc4bd] md:h-screen">
            ...
            <div className="absolute right-5 top-6 z-10 grid gap-3">
                <Button className="size-12 rounded-full bg-white text-black shadow-md hover:bg-white" size="icon">
